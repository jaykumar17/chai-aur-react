...
function App() {
  ...
  const passwordGenerator = useCallback(()=>{
    let pass = "";
    let str = "ABCDEDGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";
    ...
    <>
     ...
     <div className="w-full max-w-md mx-auto shadow-md rounded-lg px-4 my-8 text-orange-500 bg-gray-700">
        <div className="flex shadow rounded-lg overflow-hidden mb-4">
            ...
            <button className="outline-none bg-blue-700 text-white px-3 py-0.5 shrink-o" onClick={passwordToClipboard}>COPY</button>
        </div>
        <div className="flex text-sm gap-x-2">
          ...
          <div className="flex items-center gap-x-1">
            <input type="checkbox" defaultChecked={numberAllowed} id="numberInput" 
              onChange={()=>{setNumberAllowed((pre)=>!pre)}} className=""/>
              <label>Number</label>
          </div>
          <div className="flex items-center gap-x-1">
            <input type="checkbox" defaultChecked={charAllowed} id="charInput" 
              onChange={()=>{setCharAllowed((pre)=>!pre)}} className=""/>
              <label>Number</label>
          </div>
        </div>
     </div>
